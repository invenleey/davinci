...
import edp.core.debug.FTQQDebugConnector;
...
public class ViewServiceImpl extends BaseEntityService implements ViewService {
    ...
    public PaginateWithQueryColumns executeSql(ViewExecuteSql executeSql, User user) throws NotFoundException, UnAuthorizedExecption, ServerException {
        ...
        // executeSql.setSql("farmland.Role.find()");
        ...
        try {
            ...
            // TODO r3inbowari: query mongodb _id解析
            // 获取结果集
            List<Map<String, Object>> rl = paginateWithQueryColumns.getResultList();
            for (int i = 0; i < rl.size(); i++) {
                rl.get(i).put("_id", rl.get(i).get("_id").toString());
            }
    ...
    public PaginateWithQueryColumns getResultDataList(boolean isMaintainer, ViewWithSource viewWithSource, ViewExecuteParam executeParam, User user) throws ServerException, SQLException {
        ...
        try {
            ...
            if (CollectionUtils.isEmpty(querySqlList)) {
                for (String sql : executeSqlList) {
                    sql = SqlParseUtils.rebuildSqlWithFragment(sql);
                    paginate = sqlUtils.syncQuery4Paginate(sql, null, null, null, executeParam.getLimit(),
                            null);
                }
            }

            // 获取结果集
            List<Map<String, Object>> rl = paginate.getResultList();
            List<QueryColumn> cols = paginate.getColumns();
            // TODO r3inbowari mongodb _id解析
            for (int i = 0; i < rl.size(); i++) {
                rl.get(i).put("_id", rl.get(i).get("_id").toString());
            }
            ...
            for (int i = 0; i < rl.size(); i++) {
                int finalI = i;
                rl.get(i).forEach((key, obj) -> {
                    if (obj instanceof java.sql.Date) {
                        java.sql.Date da = (java.sql.Date) obj;
                        rl.get(finalI).put(key, df.format(da));
                        cols.forEach(r -> {
                    ...
                    if (obj instanceof java.util.Date) {
                        java.sql.Date da = new java.sql.Date(((java.util.Date) obj).getTime());
                        rl.get(finalI).put(key, df.format(da));
                        cols.forEach(r -> {
                            if (key.equals(r.getName())) {
                                r.setType("TIMESTAMP");
            ...
            // TODO r3inbowari mongodb sum解析
            List<Aggregator> aggregators = executeParam.getAggregators();
            for (int agg_index = 0; agg_index < aggregators.size(); agg_index++) {
                Aggregator aggregator = aggregators.get(agg_index);
                String aggregator_column = aggregator.getColumn();
                String aggregator_func = aggregator.getFunc();

                for (int i = 0; i < rl.size(); i++) {
                    rl.get(i).put(aggregator_func + "(" + aggregator_column + ")", rl.get(i).get(aggregator_column));
                }
            }

//            List<String> querySqlList = sqlParseUtils.getSqls(srcSql, true);
//            // my add
//            querySqlList = executeSqlList;
//
//            if (!CollectionUtils.isEmpty(querySqlList)) {
//                buildQuerySql(querySqlList, source, executeParam);
//                executeParam.addExcludeColumn(excludeColumns, source.getJdbcUrl(), source.getDbVersion());
...
//            }
    ...
    public List<Map<String, Object>> getDistinctValue(Long id, DistinctParam param, User user) throws
            NotFoundException, ServerException, UnAuthorizedExecption {
        ViewWithSource viewWithSource = getViewWithSource(id);
        ProjectDetail projectDetail = projectService.getProjectDetail(viewWithSource.getProjectId(), user, false);
        if (!projectService.allowGetData(projectDetail, user)) {
